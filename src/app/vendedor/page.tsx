...
import { query, collection, orderBy, limit, onSnapshot, doc, setDoc, updateDoc, getDocs, getDoc, addDoc, serverTimestamp, where, Timestamp } from "firebase/firestore";
...
export default function VendedorPage() {
  ...
  // ── Pending stamps de clientes (cliente escanea QR del local) ────────────
  const [pendingClientStamps, setPendingClientStamps] = useState<PendingClientStamp[]>([]);
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = auth.onAuthStateChanged((user) => {
      ...
      if (user) {
        ...
        // Escuchar sellos pendientes de clientes en tiempo real
        ...
        unsubscribePending = onSnapshot(pendingQ, (snapshot) => {
          ...
          snapshot.docs.forEach((d) => {
            ...
            if (!createdAt) return;
            const elapsed = (now - createdAt.toDate().getTime()) / 1000;
            if (elapsed > 5 * 60) return; // expirado
            stamps.push({ id: d.id, userId: data.userId, userName: data.userName, createdAt });
          });
          setPendingClientStamps(stamps);
        });
      }
    });
  ...
  // Auto-abrir modal cuando llega un sello pendiente nuevo
  useEffect(() => {
    if (confirmingStamp || pendingClientStamps.length === 0) return;
    const next = pendingClientStamps.find(s => !autoOpenedRef.current.has(s.id));
    if (!next) return;
    autoOpenedRef.current.add(next.id);
    setConfirmingStamp(next);
    setConfirmMonto("");
  }, [pendingClientStamps, confirmingStamp]);
  ...
  const handleConfirmClientStamp = async () => {
    if (!confirmingStamp) return;
    const montoNum = parseInt(confirmMonto.replace(/\D/g, ""), 10) || 0;
    if (montoNum <= 0 || montoNum > 150_000) {
      toast({ variant: "destructive", title: "Monto inválido", description: "Ingresa un monto entre $1 y $150.000." });
      return;
    }
  ...
  const handleRejectClientStamp = async () => {
    if (!confirmingStamp) return;
    try {
  ...
  return (
    <main className="min-h-screen bg-slate-50/50 pb-20">
        ...
        >
          ...
          >
            ...
            <div className="px-7 pt-5 pb-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ej: 5000"
                    value={confirmMonto}
                    onChange={(e) => setConfirmMonto(e.target.value.replace(/\D/g, ""))}
                    onKeyDown={(e) => e.key === "Enter" && handleConfirmClientStamp()}
              ...
              <div className="flex flex-col gap-3">
                <Button
                  onClick={handleConfirmClientStamp}
                  disabled={confirmLoading || !confirmMonto}
                  className="w-full h-14 rounded-2xl font-black text-base gap-2 shadow-lg"
                  style={{ backgroundColor: "#D3B673" }}
                >
                  {confirmLoading ? (
                ...
                <Button
                  onClick={handleRejectClientStamp}
                  disabled={confirmLoading}
                  variant="outline"
                  className="w-full h-12 rounded-2xl font-bold gap-2 text-slate-500 border-slate-200"
                >
      ...
      {/* ── Pill flotante: cliente esperando (modal cerrado manualmente) ── */}
      {pendingClientStamps.length > 0 && !confirmingStamp && (
        <button
          onClick={() => {
            const next = pendingClientStamps[0];
            autoOpenedRef.current.add(next.id);
            setConfirmingStamp(next);
            setConfirmMonto("");
          }}
        ...
        >
          ...
          <span className="font-black text-sm whitespace-nowrap">
            {pendingClientStamps[0].userName} espera sello
          </span>
